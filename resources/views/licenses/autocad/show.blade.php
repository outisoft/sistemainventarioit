    ...
    <div class="content-wrapper">
        ...
        <div class="container-xxl flex-grow-1 container-p-y">
            <h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">
                    <a href="{{ route('autocad.index') }}" class="btn-ico" data-toggle="tooltip" data-placement="top"
                        title="Regresar">
                        <span>
                            <i class='bx bx-arrow-back'></i>
                        </span>
                    </a>
                    / Licenses / AutoCAD /</span> Details </h4>
                    ...
                    <ul class="list-group list-group-horizontal-md">
                        @foreach ($licencia->equipo as $equipo)
                            <li class="list-group-item flex-fill p-6 text-body">
                                ...
                                <form
                                    action="{{ route('autocad.desasignar', ['licenciaId' => $licencia->id, 'equipoId' => $equipo->id]) }}"
                                    method="POST">
                                    @csrf
                                    @method('DELETE')
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        @if ($equipo->license->where('type_id', $licencia->type_id)->isEmpty())
                                                            <form
                                                                action="{{ route('autocad.asignar.post', ['licenciaId' => $licencia->id, 'equipoId' => $equipo->id]) }}"
                                                                method="POST">
                                                                @csrf
                                                                <button type="submit"
            ...
            @endif
            <br>
            <a href="{{ route('autocad.index') }}" class="btn btn-secondary"><i class='bx bx-arrow-back'></i>Volver</a>
