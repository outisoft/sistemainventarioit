                ...
                <div class="table-responsive text-nowrap" id="searchResults">
                    <table id="laptops" class="table">
                        ...
                        <tbody id="employeeList">
                            @foreach ($equipments as $laptop)
                                <tr>
                                    @role('Administrator')
                                        <td>{{ $laptop->region->name }} </td>
                                    @else
                                        @if ($userRegions->count() > 1)
                                            <td>{{ $laptop->region->name }} </td>
                                        @else
                                    ...
                                    <td>{{ $laptop->so }}</td>
                                    <td>
                                        @if ($laptop->lease_id && $laptop->lease)
                                            <div>
                                                Lease Code: <span
                                                    class="badge bg-label-dark">{{ $laptop->leases->lease }}</span><br>
                                                End Date: <span
                                                    class="badge bg-label-info">{{ $laptop->leases->end_date }}</span>
                                            </div>
                                        @else
                                            <span class="badge bg-label-danger">No</span>
                                            <span class="badge bg-label-dark">{{ $laptop->af_code }}</span>
                                        @endif
                                    </td>
                                    <td>
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu">
                                                <!--a-- href="#" class="dropdown-item" data-bs-toggle="modal" data-bs-target="#hojaBajaModal" data-id="{{ $laptop->id }}">
                                                    <i class='bx bx-spreadsheet'></i> Hoja de baja
                                                </!--a-->
                                                <form action="{{ route('laptops.restore', $laptop->id) }}" method="POST">
                                                    @csrf
